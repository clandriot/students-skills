...
export async function checkDefaultSkillsExist (entity) {
  let check = true
  let failedID

  if (entity.defaultSkills) {
    for (let i = 0, len = entity.defaultSkills.length; i < len && check; i++) {
      let result = await skillStorage.load(entity.defaultSkills[i].skillID)
      check = !!result
      if (!check) {
        failedID = entity.defaultSkills[i].skillID
      }
    }
  }
