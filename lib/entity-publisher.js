...
export default function publishEntity (options) {
  ...
  const {operations, additionalCreateHandler, additionalLoadHandler, additionalUpdateHandler, additionalFilterTransform, listQueryValidator, deleteCondition} = options
    ...
    server.route({
      ...
      config: {
        ...
        validate: {
          ...
          query: {
            modificationDate: joi.date().description('If provided, the patch will only be effective if the last modification date matches.')
          },
          params: {
            id: joi.trimmedString().required().description(`The ${entityName} id`)
          }
